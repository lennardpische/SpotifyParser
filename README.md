...
🎵 A small Python app that pulls my Spotify data (playlists, liked songs, recently played) into local CSVs and a growing listening-history DB so I can build personalized recs later.

- **Playlists** — names, IDs, track counts → `my_playlists.csv`
- **Playlist tracks** — every track in each playlist → `my_tracks.csv`
- **Liked songs** — saved tracks → `my_tracks.csv`
- **Recently played** — rolling window of 50 from the API, merged into a growing listening-history DB (distinct by track, no duplicates) → `my_tracks.csv` + `listening_history.json`
...
- **Seed-based recs:** I'll use `my_tracks.csv` `track_id` (and optionally artist) with [Spotify Get Recommendations](https://developer.spotify.com/documentation/web-api/reference/get-recommendations). 
